...
class OperateController extends Controller
{
    /*
     * 盆栽操作列表数据
     * params:[plant_id:盆栽id,type: 类型（0浇水，1施肥，2其他）startDate:开始时间,endDate:结束时间,page:分页]
     */
    public function index(Request $request) {
        ...
        if(isset($request->endDate)) {
            $operate = $operate->where('datetime','<=',$request->endDate);
        }
        ...
        $operate = $operate->orderBy('datetime', 'desc')->paginate(config('app.page'));
